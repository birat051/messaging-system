...
| [`mediaApi.ts`](./mediaApi.ts) | **`uploadMedia(FormData)`** → **`POST /v1/media/upload`** (multipart field **`file`**); returns **`MediaUploadResponse`** (`key`, `bucket`, optional **`url`**). Strips default JSON **`Content-Type`** so **`multipart/form-data`** gets a correct boundary. **`buildMediaUploadFormData(file)`** is in [`../utils/buildMediaUploadFormData.ts`](../utils/buildMediaUploadFormData.ts). UI code can use **`useMediaUpload`** in [`../hooks/useMediaUpload.ts`](../hooks/useMediaUpload.ts) for progress / cancel / error state. |
