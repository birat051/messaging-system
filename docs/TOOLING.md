...
- Run **`npm install`** in **`apps/web-client`**, **`apps/messaging-service`**, and **`apps/notification-service`** separately (or use the root **`npm run install:all`** helper from the root `package.json`).
...
|------|------------------|
| **web-client** | `apps/web-client/` — `tsconfig.json`, `eslint.config.mjs`, Prettier; Vite scaffold adds further config later (`TASK_CHECKLIST.md`) |
| **messaging-service** | `apps/messaging-service/` — own `package.json`, `tsconfig.json`, `eslint.config.mjs`, Prettier |
| **notification-service** | `apps/notification-service/` — own `package.json`, `tsconfig.json`, `eslint.config.mjs`, Prettier (aligned with messaging-service by copy-paste) |
...
Align copy-paste between the two backends by convention when useful; do **not** introduce a `packages/backend-tooling` shared config package unless the team explicitly changes this policy.
