...
Use this checklist to track implementation progress. Sections align with [PROJECT_PLAN.md](./PROJECT_PLAN.md). **Document order** puts **E2EE alignment** sections (**Prerequisite — User keypair**, **Feature 11**, **Bugfix — Sign out then relogin**, **Bugfix — Second browser login (sync skipped)**, **Bugfix — User A → B → A same browser (Socket.IO reconnect storm on send)**, **Feature 13**) right after **How to read**, then **MVP scope — outstanding**, then remaining **Post-MVP / extended scope**, then **Shipped**—see **[Document order](#document-order)** and **[How to read this checklist](#how-to-read-this-checklist)** (not strict build order).
...
Immediately after **How to read this checklist**, **Prerequisite — User keypair**, **Feature 11 — Message encryption**, **Bugfix — Sign out then relogin (same browser)**, **Bugfix — Second browser login (sync skipped)**, **Bugfix — User A → B → A same browser (Socket.IO reconnect storm on send)**, and **Feature 13 — Multi-device key sync** appear first so pending E2EE alignment work (per [PROJECT_PLAN.md](./PROJECT_PLAN.md) §7.1) is visible at the top. Then: **(1) MVP scope — outstanding** (Definition of done through Infrastructure and hardening), **(2) remaining Post-MVP / extended scope** (group, contacts, etc.), **(3) Shipped**. Section order is for **prioritization and scanning**, not strict build order—see **How to read this checklist**.

---

## How to read this checklist

**E2EE alignment (§7.1):** **Prerequisite — User keypair**, **Feature 11**, **Bugfix — Sign out then relogin**, **Bugfix — Second browser login (sync skipped)**, **Bugfix — User A → B → A same browser (Socket.IO reconnect storm on send)**, and **Feature 13** are placed immediately below this section so open tasks for the per-device hybrid model are easy to find. Within other groups in **[Document order](#document-order)**, headings that still contain at least one `- [ ]` appear before headings that are fully checked where applicable. Then **MVP scope — outstanding** (**Definition of done** through **Infrastructure and hardening**), then remaining **Post-MVP / extended scope**, then **Shipped**. **Implementation / dependency order** (e.g. Prerequisite → Feature 1) is still defined in [PROJECT_PLAN.md](./PROJECT_PLAN.md)—this file’s **section order** is for **prioritization and scanning**, not a prescribed build sequence.
...
- [x] **Stable repro document:** sibling **`docs/repro-second-browser-sync.md`** — steps: **browser A** = Chrome profile 1 (or Browser A); **browser B** = Chrome profile 2 / Firefox / Safari; same **`VITE_API_BASE_URL`**; confirm **both** load app from same origin rules as prod; matrix: A online vs offline when B first logs in. Cross-link from **`docs/repro-decrypt-after-relogin.md`** (**Related docs**).
- [x] **Network audit on browser B first login:** documented in **`docs/repro-second-browser-sync.md`** (**Network audit on browser B first login**) — ordered REST (**`GET …/public-keys`** → optional **`POST …/devices`** → **`GET …/devices`** → **`GET …/sync/message-keys`**) vs Socket **`connect`** (**`SessionRestore`** gates **`SocketWorkerProvider`**); **`device:sync_requested`** on **A**, **`device:sync_complete`** on **B**; blocked path when keyring empty but directory non-empty; HAR + Mermaid template.
- [x] **Capture four identifiers on browser B after login:** **`docs/repro-second-browser-sync.md`** (**Capture four identifiers on browser B after login**) — record sheet + IndexedDB **`messaging-client-crypto`** / **`deviceIdentity`** + Redux **`crypto.deviceId`** / **`syncState`** / **`registeredOnServer`** + **`GET /v1/users/me/devices`** + **`Object.keys(encryptedMessageKeys)`** vs B’s **`deviceId`**; optional **A** socket/`POST` sync bullets.
- [x] **Capture same on browser A while B logs in:** **`docs/repro-second-browser-sync.md`** (**Capture on browser A while B logs in**) — WS / Redux **`pendingSync`** / **`emitDeviceSyncRequested`**; **`DeviceSyncApprovalBanner`** (**`data-testid`** + guest gate); **`useDeviceKeySync`** **`POST …/sync/message-keys`** (with refresh **`sourceDeviceId`**, pagination).

### (2) Root-cause analysis — web-client

- [x] **Bootstrap trace on B:** **`docs/repro-second-browser-sync.md`** (**Bootstrap trace on browser B (root-cause — web-client)**) — **`bootstrapSessionIfNeeded`** / **`SessionRestore`** + **`useSenderKeypairBootstrap`** → **`ensureUserKeypairReadyForMessaging`** (**`ensureMessagingKeypair.ts`**) → **`POST …/devices`** → **`evaluateDeviceSyncBootstrapState`**; **`syncState`** table; **`pending`** from **`GET …/sync/message-keys`** (not thread fetch); **`message:new`** re-eval; errors → **`idle`**; blocked path before **`POST`**.
- [x] **Socket path on A:** **`docs/repro-second-browser-sync.md`** (**Socket path on A (trusted device)**) — **`socketWorker.ts`** → **`device_sync_requested`** → **`SocketWorkerProvider`** **`syncRequested`** + self-**`deviceId`** filter; **`cryptoSlice.pendingSync*`**; **`DeviceSyncApprovalBanner`** uses **`selectPendingSync`** + non-guest only — **not** **`syncState`**; **`device:sync_complete`** on B **does** gate on **`syncState`**.
- [x] **Sync orchestrator:** **`docs/repro-second-browser-sync.md`** (**Sync orchestrator (`useDeviceKeySync`)**) — **`executeApproveDeviceKeySync`**: **`listMyDevices`** gate → **`listMySyncMessageKeys(sourceDeviceId)`** pagination → **`unwrapMessageKey`** / **`wrapMessageKey`** → **`postBatchSyncMessageKeys({ targetDeviceId: newDeviceId })`**; **`targetDeviceId`** = Socket **`newDeviceId`** = **`GET …/devices`** row for B; refresh with **`sourceDeviceId`** (A).
- [x] **New device UI:** **`docs/repro-second-browser-sync.md`** (**New device UI (`NewDeviceSyncBanner`)**) — **`pending`** \| **`in_progress`**; copy + device list for **B** → open **A**; **non-blocking** (no modal); **`data-testid`** / SWR / **`evaluateDeviceSyncBootstrapState`** refresh.
- [x] **Decrypt path on B:** **`docs/repro-second-browser-sync.md`** (**Decrypt path on browser B (peer inbound)**) + **`docs/repro-decrypt-after-relogin.md`** (table **UI errors → code paths**) — **`NO_LOCAL_KEY`** long line vs **`NO_DEVICE_KEY_ENTRY`** short line; ambiguity + **`VITE_DEBUG_PEER_DECRYPT`**.

### (3) Root-cause analysis — messaging-service

- [x] **`POST /users/me/devices`:** when B registers, verify **`device:sync_requested`** is emitted to **`user:<userId>`** room and payload includes **`newDeviceId`** + **`newDevicePublicKey`**; check **Socket.IO** adapter and **JWT** **`deviceId`** / **`sourceDeviceId`** on sync **POST** bodies (**auth** middleware). — **Verified in code + doc:** **`docs/repro-second-browser-sync.md`** § **`POST /users/me/devices` emit + sync JWT (`sourceDeviceId`)** (claim is **`sourceDeviceId`**, not **`deviceId`**; **`GET …/sync/message-keys`** uses query **`deviceId`**).
- [x] **`POST /users/me/sync/message-keys`:** authz (**caller device** must match registered device); Mongo **`$set`** on **`encryptedMessageKeys.<targetDeviceId>`**; errors returned to client (403 vs 429 vs 400). — **Verified:** **`docs/repro-second-browser-sync.md`** § **`POST /users/me/sync/message-keys` authz + Mongo + errors** (`syncMessageKeys.ts`, **`users.ts`** **`postBatchSyncMessageKeys`** / **`rateLimitDeviceSyncBatch`**, **`validation/middleware.ts`**).
- [x] **Rate limits:** **`DEVICE_SYNC_RATE_LIMIT_*`** — confirm B’s sync is not blocked after A approves. — **Verified:** limit applies only to **`POST /users/me/sync/message-keys`**; only **`useDeviceKeySync.executeApproveDeviceKeySync`** (trusted **A**) calls **`postBatchSyncMessageKeys`** — **B** does not, so B is not blocked by this cap after approval (see **`docs/repro-second-browser-sync.md`** § **`DEVICE_SYNC_RATE_LIMIT_*` vs browser B**).

### (4) Fix / product alignment (after root cause)

- [x] **Policy:** document expected UX — **browser B** must either complete **Feature 13** sync from **A**, or only see **new** messages that include **`encryptedMessageKeys[B]`**; historical ciphertext **cannot** decrypt without sync — align copy in banners with **`docs/PROJECT_PLAN.md`** §7.1. — **`docs/repro-second-browser-sync.md`** § **Expected UX policy — Feature 13 vs ciphertext**; **`NewDeviceSyncBanner`**, **`DeviceSyncApprovalBanner`**, **`peerDecryptInline`** **`PEER_DECRYPT_NO_DEVICE_KEY_ENTRY`** aligned with **`docs/PROJECT_PLAN.md` §7.1**.
- [x] **Regression tests:** extend **`logoutReloginHybridDecrypt.integration.test.ts`** patterns or add **`secondBrowserSync.integration.test.ts`** — mock Socket event + batch **POST**, assert **`encryptedMessageKeys`** gains new device entry (where feasible without full Playwright). — **`apps/web-client/src/common/integration/secondBrowserSync.integration.test.ts`** ( **`DeviceSyncRequestedPayload`**, **`BatchKeyUploadRequest`**-shaped **`POST`**, **`applyBatchSyncToEncryptedMessageKeys`**, real **`unwrapMessageKey`** / **`wrapMessageKey`** / **`decryptHybridMessageToUtf8`**); **`logoutReloginHybridDecrypt.integration.test.ts`** header cross-link.
...
## Bugfix — User A → B → A same browser (Socket.IO reconnect storm on send)

**Symptom (reported flow):** **User A** (new device on this browser) sends/receives → **logout** → **User B** logs in (device id from another device) → completes **Feature 13** sync → sends/receives → **logout B** → **User A** logs in again **same browser** → **historical messages visible** → on **send**, Socket.IO **`connectSocket`** runs in a **tight loop** (worker **`generation`** 7, 8, 9, …); server logs repeat **`POST /v1/auth/refresh`** → **`client namespace disconnect`** → **new socket.io connected**. Sending may still **succeed** (`message:send` ack `ok`) while reconnect spam continues.

**Likely themes:** Redux **`accessToken`** churn after **user-switch / session bootstrap** (`setSession`, **`sessionBootstrap`**, **`upgradeAccessTokenWithDeviceBinding`**) triggers **`SocketWorkerProvider`** **`updateAccessToken`** → worker **`connectSocket`**; parallel REST during **`useSendEncryptedMessage`** (**`GET …/devices/public-keys`**, etc.) triggers **`POST /auth/refresh`**; **no coalescing** of “same JWT” updates or **refresh mutex** firing multiple **`setSession`** writes; optional **IndexedDB / `sourceDeviceId`** thrash when switching back to A.

### (1) Reproduce & narrow

- [x] **Stable repro doc:** **`docs/repro-user-a-b-a-socket-reconnect.md`** — exact steps: A new → B other device + sync → B logout → A relogin → open thread → send one message; **browser**, **`VITE_*`** matrix, **`VITE_REVOKE_DEVICE_ON_LOGOUT`**; cross-link from **`docs/repro-second-browser-sync.md`**.
- [x] **Correlate timelines:** runbook — **`docs/repro-user-a-b-a-socket-reconnect.md`** § **Correlating timelines (one browser session)** — **`sessionWriteDebug`** / Network **`POST /auth/refresh`** / server **`socket.io`** lifecycle + merge table template.
- [x] **Count refresh sources:** **`apps/web-client/src/modules/auth/utils/sessionWriteDebug.ts`** — **`[DEBUG]: auth session write`** with **`source`** + **`accessTokenFingerprint`** (FNV-ish hash, not JWT text) + **`duplicateOfPreviousWrite`**; **`setSession`** write paths — **`applyAuthResponse`** (**`sessionWriteSource`** — **`auth.login`**, **`auth.register`**, **`sessionBootstrap.refresh`**, **`sessionBootstrap.deviceBoundRefresh`**, **`crypto.upgradeAccessTokenWithDeviceBinding`**, **`crypto.deviceKeySync.approveSync`**, default **`applyAuthResponse`**), **`applyGuestAuthResponse`**, **`applyVerifyEmailResponse`**, **`httpClient.performRefresh`** (401 mutex, bypasses **`applyAuthResponse`**). After **A’s second login** expect at least **`auth.login`** and often **`sessionBootstrap.refresh`** (+ optional **`sessionBootstrap.deviceBoundRefresh`**) before first send; parallel **401**s add **`httpClient.performRefresh`**.

### (2) Trace — web-client

- [x] **Map token → socket path:** **`docs/repro-user-a-b-a-socket-reconnect.md`** § **Map token → socket path (vs `message_send`)** — **`setSession`**; bootstrap **`useEffect([userId, dispatch, reduxStore])`** + token **`useEffect([userId, accessToken])`** (**350 ms** debounce) → **`bridge.updateAccessToken`** → **`update_access_token`** → **`connectSocket`**; ordering vs encrypt / **`message_send`** + worker queue.
- [x] **User-switch hygiene:** **`docs/repro-user-a-b-a-socket-reconnect.md`** § **User-switch hygiene** — **`logout`** Redux resets (**`crypto`** / **`devicePublicKeys`** / **`messaging`** / **`presence`** via **`logout`**); **`SocketWorkerProvider`** single React instance (**`main.tsx`**), worker **teardown + one `connect` per `userId`** transition; debounce **`clearTimeout`** on **`userId`** empty / deps change / unmount (**no orphan `updateAccessToken` timers**); **`SessionRestore`** bootstrap once on mount (not per login); **`StrictMode`** dev double-effect caveat.
- [x] **Reduce unnecessary reconnects:** **`SocketWorkerProvider`** **`lastWorkerAccessTokenRef`** — skip **`bridge.updateAccessToken`** when Redux token **string** equals last **`connect`/update** ( **`apps/web-client/src/common/realtime/SocketWorkerProvider.tsx`** ); worker dedupe unchanged (**`socketWorker`**). **Optional:** reconnect only on socket **401** — tradeoffs in **`docs/repro-user-a-b-a-socket-reconnect.md`** § **Optional future — reconnect only on socket auth failure**.

### (3) Trace — messaging-service (optional)

- [x] **Confirm server-side pairing:** **`docs/repro-user-a-b-a-socket-reconnect.md`** § **Server-side pairing** — **`postRefresh`** does not touch **`Socket.IO`**; **`socket.io disconnected`** **`reason`** + new **`socket.io connected`** align with **client** reconnect after JWT apply; storms → reduce **client** refresh/reconnect frequency.

### (4) Fix implementation

- [x] **Mitigations shipped (baseline):** debounced **`updateAccessToken`** (**`SocketWorkerProvider`**); worker **send queue** until **`connect`**; **`socketConnectGeneration`** guards; synthetic **`disconnected`** before replacement; **`applyPresenceStatus`** syncs Redux + Context without **`useEffect`** lag — see **`socketWorker.ts`**, **`SocketWorkerProvider.tsx`**.
- [x] **Stop the storm:** **`docs/repro-user-a-b-a-socket-reconnect.md`** § **Stop the storm — causes & mitigations**; **`authSlice.setSession`** **`authSessionEquivalent`** no-op (same JWT + expiry + user id + guest); **`SocketWorkerProvider`** **`DEBOUNCE_MS` 350** + existing **`lastWorkerAccessTokenRef`**; **`httpClient`** refresh mutex already single-flight per **401** wave.
- [x] **Regression tests:** **`authSlice.test.ts`** (coalesce / bootstrap handoff); **`SocketWorkerProvider.test.tsx`** — identical token → **`updateAccessToken`** not called after debounce window.

### (5) Verification

- [ ] **Manual QA:** run full **A → B → A** matrix; **send** after relogin A — **≤1** socket replacement per logical token rotation; **no** endless reconnect loop in server logs / UI; server shows **≤1** refresh-driven reconnect per send unless token actually expired.
- [x] **Cleanup:** removed **`[DEBUG]`** `console.log` from **`socketWorker.ts`**, **`socketBridge.ts`**, **`useSendMessage.ts`**, **`SocketWorkerProvider.tsx`**, **`useSendEncryptedMessage.ts`** (send stack); **`sessionWriteDebug`** retained for token-write tracing; **`SocketWorkerProvider`** **`message:new`** **`console.warn`** kept for invalid payloads.
